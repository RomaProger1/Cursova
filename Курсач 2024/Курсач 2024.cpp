...
void searchParameters() {
	...
	bool SearchSelection = false; // Частично совпадающие переменные 
	bool recursSeh = false;// Поиск по всем директориям
	...
	try{
		...
		for (const auto& entry : fs::recursive_directory_iterator(directory)) {
			if (entry.is_regular_file()) {
				...
				string current_filename = wstring_to_string(current_wfilename);
...
void logic() {
	...
	if (choice == 'a'|| choice == 'b' || choice == 'c'|| choice == 'd') {
		...
		if (selectedDrive !='\0'){
			...
			{
				cout << "Введите искомый текст: " << endl;
			}
			else{
				searchParameters();
			...
			try{
				if (choice == 'a') {
					...
					Metric::searchTextFiles(strWhyChoice, filenameOr);
				}
				else {
